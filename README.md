...
| `999-qos-18`, `999-qos-19` | AQM v2: syncs the generic `flow_offload_teardown()` on eviction instead of only clearing the PPE hardware entry, and a congestion-aware ct-mark hold/release table (`hold_ms`, opt-in — save/restore and hold-duration correctness both confirmed live) so an evicted flow's re-offload timing is an AQM decision, not nf_flowtable's generic 30s idle GC — flashed and hardware-tested 2026-09-06. A properly-powered 4-rep A/B found no statistically distinguishable throughput/latency/retransmit cost from `hold_ms`; still ships disabled by default (no measured benefit yet either, at this household's current light traffic level). See [`docs/e8450-aqm-v2-design.md`](docs/e8450-aqm-v2-design.md) |
