...
| | |
|---|---|
| Kernel | Linux 6.12.103, revision `r33092+5-12141b5981`, live-flashed and hardware-verified |
| Local patch count | 44 (`target/linux/mediatek/patches-6.12/999-*.patch`) + 2 mt76-specific + 3 custom packages — all 44 flashed and live and hardware-tested |
...
| QoS/AQM | Production HQoS+AQM profile live: hardware leaky-bucket WAN shaping (`q7`/`q8`) + software occupancy-driven eviction, byte-accurate, flow-aware, `grace_ms=1000`, `hold_ms=3000` (AQM v2, congestion-aware ct-mark hold/release — adopted 2026-09-06 after an 8-rep multi-stream A/B found no measured cost and a plausible tail-latency benefit) |
...
| `999-qos-18`, `999-qos-19` | AQM v2: syncs the generic `flow_offload_teardown()` on eviction instead of only clearing the PPE hardware entry, and a congestion-aware ct-mark hold/release table (`hold_ms`) so an evicted flow's re-offload timing is an AQM decision, not nf_flowtable's generic 30s idle GC — flashed and hardware-tested 2026-09-06. `hold_ms=3000` **adopted as the production default** (`qdma-shaper.config`) after an 8-rep multi-stream A/B found no statistically distinguishable throughput/latency/retransmit cost and flatter/lower worst-case latency, including one real household-congestion event apparently contained only with the hold active. See [`docs/e8450-aqm-v2-design.md`](docs/e8450-aqm-v2-design.md) |
...
| `999-ppe-13` | Removes a multicast IB2 bit that gave the CDMA an incorrect PPE CPU reason on multicast packets (mDNS/SSDP/IGMP traffic) — flashed and smoke-tested clean; no dedicated multicast-traffic test yet. See [`docs/e8450-mtk-feeds-audit-2026-09.md`](docs/e8450-mtk-feeds-audit-2026-09.md) |
...
| Patch | Purpose |
|---|---|
| `999-dsa-06` | Fixes `mt7530_hw_vlan_del()` to set `FID(FID_BRIDGED)` on a VLAN entry that still has member ports after a deletion, matching the FID the `_add()` path already sets — flashed and smoke-tested clean; no dedicated VLAN-cycling test yet. See [`docs/e8450-mtk-feeds-audit-2026-09.md`](docs/e8450-mtk-feeds-audit-2026-09.md) |
...
| `999-eth-53` | Fixes a `mtk_mdio_busy_wait()` race that could report a false MDIO timeout on a hardware access that actually succeeded — flashed and smoke-tested clean; no dedicated MDIO-stress test yet. See [`docs/e8450-mtk-feeds-audit-2026-09.md`](docs/e8450-mtk-feeds-audit-2026-09.md) |
