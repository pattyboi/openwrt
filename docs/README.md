...
| [`e8450-mtk-feeds-audit-2026-09.md`](e8450-mtk-feeds-audit-2026-09.md) | Audit of `mediatek/mtk-openwrt-feeds` and `immortalwrt/immortalwrt` for anything new and useful not already in this tree: five verified-applicable findings (multicast PPE CDMA bug, MDIO timeout race, MT7531 VLAN FID bug, PPE conntrack-mark offload bypass, TCP-window-check interaction with AQM eviction), two A/B candidates (NAPI weight, watchdog timeout clamp), and a longer ruled-out list with the version-gate/source evidence for each.
