...
| [`e8450-aqm-v2-design.md`](e8450-aqm-v2-design.md) | AQM v2 design: closes the gap where evicted flows' re-offload timing was governed by nf_flowtable's generic 30s idle GC instead of the AQM itself. Implemented, build-verified, flashed, and hardware-tested 2026-09-06 (`999-ppe-94`/`999-qos-18`/`999-qos-19`): syncs `flow_offload_teardown()` on eviction (unconditional, no regression observed), a congestion-aware ct-mark hold/release table (`hold_ms`, save/restore correctness confirmed live — ships opt-in/default-off pending a larger retransmit A/B, one noisy sample showed more retransmits with it active), and a defensive PPE queue-range clamp. Includes a survey of every mainline hardware-offload driver for eviction precedent (none exists) and a real build-time discovery (a built-in-vs-module link failure, fixed with `symbol_get()`). |
